...
replacement = """                <div className="absolute inset-0 bg-black/40 pointer-events-none transition-opacity duration-700" />
                <div className={`absolute inset-0 transition-opacity duration-700 pointer-events-none ${isHovered ? 'bg-gradient-to-t from-black/95 via-black/60 to-transparent' : 'bg-gradient-to-t from-black/90 via-black/50 to-transparent'}`} />
                <div className="absolute inset-0 p-4 md:p-6 flex flex-col justify-end pointer-events-none z-10">
                  <span className={`text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] text-white drop-shadow-lg mb-2`}>
                    {item.category}
                  </span>
                  <div className={`overflow-hidden max-h-24 opacity-100`}>
                    <p className={`text-white/90 font-medium text-xs md:text-sm leading-relaxed drop-shadow-md`}>
                      {item.detail}
                    </p>
                  </div>
                </div>"""
